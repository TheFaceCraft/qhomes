<!-- Dashboard -->
@if(auth()->user()->hasPermission('dashboard.view') || auth()->user()->isSystemSuperAdmin())
...
<!-- System Super Admin Menu Items -->
@if(auth()->user()->isSystemSuperAdmin())
    <!-- Company Users Management -->
    <x-admin-nav-link :href="route('admin.company-users.index')" :active="request()->routeIs('admin.company-users.*')">
        <svg class="text-gray-400 mr-3 flex-shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <!-- System Statistics -->
    <x-admin-nav-link :href="route('admin.stats')" :active="request()->routeIs('admin.stats')">
        <svg class="text-gray-400 mr-3 flex-shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
@else
    <!-- Properties (Not for Super Admin) -->
